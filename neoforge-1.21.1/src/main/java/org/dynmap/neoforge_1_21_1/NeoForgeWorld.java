...
/**
 * NeoForge specific implementation of DynmapWorld
 */
...
public class NeoForgeWorld extends DynmapWorld {
	...
	public void updateWorld(ServerLevelAccessor w) {
		this.updateWorldHeights(w.getLevel().getHeight(), w.getLevel().dimensionType().minY(),
				w.getLevel().getSeaLevel());
	}

	public NeoForgeWorld(ServerLevelAccessor w) {
		this(getWorldName(w),
	...
	public NeoForgeWorld(String name, int height, int sealevel, boolean nether, boolean the_end, String deftitle,
			int miny) {
		...
		// Log.info(getName() + ": skylight=" + skylight + ", height=" +
		// this.worldheight + ", isnether=" + isnether + ", istheend=" + istheend);
	}
	...
	public MapChunkCache getChunkCache(List<DynmapChunk> chunks) {
		if (world != null) {
			NeoForgeMapChunkCache c = new NeoForgeMapChunkCache(DynmapPlugin.plugin.sscache);
